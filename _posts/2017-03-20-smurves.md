...
Given a user-specified set of very flexible constraints, the generator can be used in two ways to either create random functions with certain characteristics or perturb existing functions. For the first application, the generated curves can be directly used as random function instances, which is first introduced in our [paper](https://arxiv.org/abs/1812.09786) on the viability of type SN Ia supernovae for testing the standard model of cosmology. The second application treats the specified number of projectile location measurements as multiplicators for the values of a function that is to be smoothly perturbed.
