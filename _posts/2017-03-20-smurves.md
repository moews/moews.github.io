...
Given a user-specified set of very flexible constraints, the generator can be used in two ways to either create random functions with certain characteristics or perturb existing functions. For the first application, the generated curves can be directly used as random function instances, which is first introduced in our [paper](https://arxiv.org/abs/1812.09786) on the viability of type Ia supernovae for testing the standard model of cosmology, quickly followed by its use in an emulator for the non-linear matter power spectrum of non-standard cosmologies in a second [paper](https://arxiv.org/abs/1906.02742). The second application treats the specified number of projectile location measurements as multiplicators for the values of a function that is to be smoothly perturbed.
